...
<main class="zomeex-quote" id="main-content">
	...
	<div class="zomeex-container">
		...
		<?php else : ?>
			...
			<form class="zomeex-quote__layout" data-quote-form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
				<div class="zomeex-quote__brief">
					...
					<div class="zomeex-quote__fieldset"><p class="zomeex-kicker">Project context</p><h2>Where should we focus?</h2><div class="zomeex-form-grid"><label><span>Name <em>*</em></span><input type="text" name="name" autocomplete="name" required></label><label><span>Company <em>*</em></span><input type="text" name="company" autocomplete="organization" required></label><label><span>Work email <em>*</em></span><input type="email" name="email" autocomplete="email" required></label><label><span>Country / region <em>*</em></span><input type="text" name="country" autocomplete="country-name" required></label><label><span>Your role</span><select name="role"><option value="">Select one</option><option>Founder / owner</option><option>Procurement</option><option>Product / R&amp;D</option><option>Brand / marketing</option><option>Compliance / legal</option><option>Distributor</option><option>Other</option></select></label><label><span>Target market <em>*</em></span><input type="text" name="target_market" placeholder="e.g. EU, US, Canada" required></label><label><span>Estimated quantity</span><input type="text" name="quantity" inputmode="numeric" placeholder="Units per order or year"></label><label><span>Target timeline</span><select name="timeline"><option value="">Select one</option><option>Exploring options</option><option>1-3 months</option><option>3-6 months</option><option>6+ months</option></select></label></div></div>
					<div class="zomeex-quote__fieldset"><p class="zomeex-kicker">Build requirements</p><h2>What should we prepare?</h2><div class="zomeex-form-grid"><label class="zomeex-form-field--full"><span>Customization, finish or packaging</span><textarea name="customization" rows="4" placeholder="Colors, branding, format, technical requirements"></textarea></label><label><span>Samples</span><select name="samples"><option value="">Select one</option><option>Yes, please advise</option><option>Not yet</option><option>Already have samples</option></select></label><label><span>Anything else</span><textarea name="notes" rows="3" placeholder="Additional context"></textarea></label></div></div>
				</div>
				<aside class="zomeex-quote__aside"><div class="zomeex-quote__aside-inner"><p class="zomeex-kicker">Before you send</p><h2>A useful brief makes the next reply sharper.</h2><ul><li>Destination market and expected volume</li><li>Product family or format</li><li>Branding, packaging or equipment needs</li><li>Sample and timing expectations</li></ul><p class="zomeex-quote__privacy">Your details are used to respond to this request. We do not publish your brief.</p><input type="hidden" name="action" value="zomeex_quote_submit"><input type="hidden" name="quote_items" id="zomeex-quote-items" value="[]"><?php wp_nonce_field( 'zomeex_quote_submit', 'zomeex_quote_nonce' ); ?><button class="zomeex-button zomeex-button--solid zomeex-button--wide" type="submit">Send quote request <span aria-hidden="true">↗</span></button></div></aside>
			</form>
		<?php endif; ?>
	</div>
