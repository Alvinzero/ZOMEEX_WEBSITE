		...
		<?php else : ?>
			...
			<form class="zomeex-quote__layout" data-quote-form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
				<div class="zomeex-quote__brief">
					...
					<div class="zomeex-quote__fieldset"><p class="zomeex-kicker">Build requirements</p><h2>What should we prepare?</h2><div class="zomeex-form-grid"><label class="zomeex-form-field--full"><span>Customization, finish or packaging</span><textarea name="customization" rows="4" maxlength="3000" placeholder="Colors, branding, format, technical requirements"></textarea></label><label><span>Samples</span><select name="samples"><option value="">Select one</option><option>Yes, please advise</option><option>Not yet</option><option>Already have samples</option></select></label><label><span>Anything else</span><textarea name="notes" rows="3" maxlength="3000" placeholder="Additional context"></textarea></label></div></div>
				</div>
				<aside class="zomeex-quote__aside"><div class="zomeex-quote__aside-inner"><p class="zomeex-kicker">Before you send</p><h2>A useful brief makes the next reply sharper.</h2><ul><li>Destination market and expected volume</li><li>Product family or format</li><li>Branding, packaging or equipment needs</li><li>Sample and timing expectations</li></ul><p class="zomeex-quote__privacy">Your details are used to respond to this request. We do not publish your brief.</p><div class="zomeex-honeypot" aria-hidden="true" hidden><label>Website<input type="text" name="zomeex_quote_honeypot" tabindex="-1" autocomplete="off"></label></div><input type="hidden" name="action" value="zomeex_quote_submit"><input type="hidden" name="quote_items" id="zomeex-quote-items" value="[]"><?php wp_nonce_field( 'zomeex_quote_submit', 'zomeex_quote_nonce' ); ?><button class="zomeex-button zomeex-button--solid zomeex-button--wide" type="submit" data-quote-submit><span data-quote-submit-label>Send quote request</span> <span aria-hidden="true">↗</span></button></div></aside>
			</form>
		<?php endif; ?>
	</div>
